...
def lambda_handler(event, context):
    ...
    collage_lists = [[None] * len(emotions)]  # create lists for emotions sort
    for i, key in enumerate(keys):  # cutout all faces from each image
        ...
        for j, face in enumerate(tmp_faces):
            ...
            collage_lists[emotions.index(tmp_emotions[j])].append(im_crop)  # add image to dedicated sub-list


    for i, collection in enumerate(collage_lists):
        if len(collection) > 0:
